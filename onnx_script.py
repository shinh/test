...
def make_constant_sequence_node(name, typ, values):
    ...
    raise RuntimeError('Sequence constant not supported')
...
class GraphBuilder(object):
    ...
    def gen_test(self, graph=None, validate=False):
        if graph is None:
            graph = self.make_graph()
        outputs = self.outputs + self.gradients
        gen_test(graph, self.inputs, outputs,
                 name=self.graph_name, validate=validate)
